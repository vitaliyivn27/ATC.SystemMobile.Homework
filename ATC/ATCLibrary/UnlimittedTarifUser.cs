...
{
    public class UnlimittedTarifUser : User
    {
        public UnlimittedTarifUser(decimal summury) : base(summury)
        {
        }
        protected internal override void Open()
        {
            base.OnOpened(new UserEventArgs($"Opened new account with unlimitted tarif! Number: {this.Number}", this.Sum));
        }
        public void AddOrCloseUnlimittedNumber(decimal summury)
        {
            if (Sum >= 0)
            {
